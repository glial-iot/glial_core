...
В режиме IMPACT все данные, поступающие с драйверов хранятся в облачном хранилище [Nokia IMPACT](https://networks.nokia.com/solutions/iot-platform)  <!--Это не облачное хранилище, они туда просто пересылаются-->
...
## Drivers <!--Функции для работы с логами и шиной и переменные скрипта относятся ко всем скриптам, не только к драйверам-->
...
Общая шина — это быстрая in-memory база данных ключ:значение, в которой ключом является стандартизованный адрес устройства или датчика, а значением — последние данные с этого устройства или датчика. <!--а еще дополнительно хранится метаинформация, тоже написать надо-->
...
Scheduled - запланированное время в формате HH:mm или DD:HH:MM ? (1 раз или сколько?) -->
...
<!-- написать про то, как устанавливаются endpoints в веб-интерфейсе и что к ним добавляется /we/-->
