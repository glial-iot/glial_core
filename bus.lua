...
function bus_local.events_handler(topic, value)
...
function bus_local.get_tsdb_save_attribute(topic)
...
function bus_local.fifo_storage_worker()
   while true do
      local key, topic, timestamp, value = bus_local.fifo_get_delete_value()
      ...
      if (key ~= nil) then
         ...
         local tsdb_save = bus_local.get_tsdb_save_attribute(topic)
...
function bus_local.bus_rps_stat_worker()
   while true do
...
function bus.init()
   ...
   bus_local.fifo_storage_worker_fiber = fiber.create(bus_local.fifo_storage_worker)
   bus_local.bus_rps_stat_worker_fiber = fiber.create(bus_local.bus_rps_stat_worker)
end

function bus.update_value(topic, value) -- external value name (incorrect)
   bus_local.add_value_to_fifo_buffer(topic, value)
end

function bus.delete_value(topic)
   if (topic ~= nil) then
      bus.bus_storage.index.topic:delete(topic)
   end
end

function bus.set_tsdb_save_attribute(topic, value)
...
function bus.action_data_handler(req)
   ...
   if (param_action == "delete_value") then
      bus.delete_value(param_topic)
   elseif (param_action == "update_tsdb_attribute") then
      ...
      bus.set_tsdb_save_attribute(param_topic, param_value)
...
function bus.http_data_handler(req)
   ...
   -- 1(topic), 2(timestamp), 3(value), 4(tsdb_save)
   for _, tuple in bus.bus_storage.index.topic:pairs() do
      i = i + 1
      data_object[i] = {}
