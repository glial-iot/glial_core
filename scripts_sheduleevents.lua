...
function shedule_events_private.unload(uuid)
   ...
   if (body.destroy ~= nil) then
      ...
      if (destroy_returned_data == false) then
         log_shedule_events_warning('Shedule-event script "'..script_params.name..'" not stopped, need restart glial', script_params.uuid)
      ...
      if (script_table ~= nil) then
         if (script_table.status ~= scripts.statuses.STOPPED) then
            ...
            if (script_table.unload_result == true) then
               ...
               log_shedule_events_warning('Timer-event script "'..script_table.name..'" not deleted(not stopped), need restart glial', script_table.uuid)
