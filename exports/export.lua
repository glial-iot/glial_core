   ...
   if (params["action"] == "set") then
      if (params["type"] == "influx") then
         if (params["value"] == "true" or params["value"] == "false") then
            ...
            return_object = req:render{ json = {result = false, error_msg = "Export API set: No valid value"} }
         end
      ...
      elseif (params["type"] == "impact") then
         ...
         return_object = req:render{ json = {result = false, error_msg = "Export API set: No valid type"} }
      end

   elseif (params["action"] == "get") then
      if (params["type"] == "influx") then
         local value = influx.get_status()
         return_object = req:render{ json = {result = true, value = value} }
      elseif (params["type"] == "impact") then
      ...
      return_object = req:render{ json = {result = false, error_msg = "Exports API: No valid action"} }
   end
